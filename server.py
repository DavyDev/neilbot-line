...
if channel_secret is None or channel_access_token is None:
    print('Specify CHANNEL_SECRET and ACCESS_TOKEN as environment variables.')
    sys.exit(1)
...
@app.route(os.getenv('CALLBACK_URL', None), methods=['POST'])
def callback():
...
def handle_text_message(event):
    ...
    if text == 'profile':
        ...
        line_bot_api.reply_message(
            event.reply_token, [
                TextSendMessage(
                    text='link_token: ' + link_token_response.link_token)
            ])
        ...
        if response.status == 'ready':
            messages = [
                ...
                TextSendMessage(
                    text='targetedReaches: ' + str(response.targeted_reaches)),
...
def handle_content_message(event):
    ...
    with tempfile.NamedTemporaryFile(
            dir=static_tmp_path, prefix=ext + '-', delete=False) as tf:
...
def handle_file_message(event):
    message_content = line_bot_api.get_message_content(event.message.id)
    with tempfile.NamedTemporaryFile(
            dir=static_tmp_path, prefix='file-', delete=False) as tf:
        for chunk in message_content.iter_content():
            tf.write(chunk)
        tempfile_path = tf.name
...
def call_at_interval(time, callback, args):
    while True:
        log.info('confirming the app-alive status..')
...
if __name__ == "__main__":
    ...
    parsConf = arg_parser.parse_args()
